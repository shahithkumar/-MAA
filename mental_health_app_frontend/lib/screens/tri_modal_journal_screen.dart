...
class TriModalJournalScreen extends StatefulWidget {
  final List<CameraDescription> cameras;
  const TriModalJournalScreen({required this.cameras, super.key});

  @override
  State<TriModalJournalScreen> createState() => _TriModalJournalScreenState();
}

class _TriModalJournalScreenState extends State<TriModalJournalScreen> with TickerProviderStateMixin {
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              "We’ll gently analyze facial expressions to understand your emotional state.\n\nNothing is stored permanently. The image is discarded immediately after analysis.",
  ...
  Future<void> _analyzeAndSave() async {
    if (_textController.text.isEmpty && _voiceBytes == null && _imageBytes == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Please share something first (text, voice, or face).")),
      );
      return;
    }
    ...
    try {
      final result = await ApiService().saveTriModalJournal(
        text: _textController.text,
        voiceBytes: _voiceBytes,
        faceBytes: _imageBytes,
      );

      if (mounted) {
        setState(() {
          _finalEmotion = result["emotion"] ?? "neutral";
          _confidence = result["confidence"]?.toDouble() ?? 0.5;
          _components = result["components"]; // Store detailed components
          _isUploading = false;
        });
        _showResultModal();
      }
                 ...
                 children: [
                   ...
                   Container(
                     ...
                     child: FractionallySizedBox(
                       ...
                       widthFactor: confidence, // Ensure 0.0 - 1.0
                       child: Container(
                         decoration: BoxDecoration(
                           color: emotionColor,
                           borderRadius: BorderRadius.circular(3),
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            // FACE CHIP
                            _buildModeChip(
                              label: _imageBytes != null ? "Face Added" : "Face",
                              icon: _imageBytes != null ? Icons.check_rounded : Icons.face_rounded,
                              isActive: _imageBytes != null,
                              isRecording: false,
                              onTap: _imageBytes != null ? () {} : _showFaceTrustModal,
                            ),
                          ],
                        ),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                         ...
                         const SizedBox(height: 16),
                        Text(
                          "We'll capture a frame automatically.",
                          textAlign: TextAlign.center,
                          style: GoogleFonts.outfit(fontSize: 16, color: Colors.white70),
                        ),
                        const SizedBox(height: 40),
                        // Manual Capture Button (or could be auto)
                        ...
                        const SizedBox(height: 24),
                        TextButton(
                          onPressed: () => setState(() => _isCapturingFace = false),
                          child: Text("Cancel", style: GoogleFonts.outfit(color: Colors.white70, fontSize: 16)),
                        )
                      ],
                    ),
                  ),
